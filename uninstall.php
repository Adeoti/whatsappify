 ...
 */
 ...
 function cleanup_wafy_data(){
    ...
    $wafy_delete_nonce = get_option('wafy_delete_nonce', '');

    if(!empty($wafy_delete_nonce) && check_admin_referer( $wafy_delete_nonce)){
        $wafy_options = [
            'wafy_wid_agents',
            ...
            global $wpdb;
            $wpdb->query($wpdb->prepare("DELETE FROM $wpdb->postmeta WHERE meta_key LIKE %s", 'wafy_%'));
